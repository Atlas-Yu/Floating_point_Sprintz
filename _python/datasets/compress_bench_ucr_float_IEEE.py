...
def _quantize(mat, dtype, axis=0):
    ...
    # First quantize to float 16
    max_float16 = np.finfo(np.float16).max
    mat = (mat * max_float16).astype(np.float16)
    mat = np.apply_along_axis(split_float16, 1, mat)
    return mat.astype(dtype)


def split_float16(row):
    res = []
    # Pack a float16 into byte objects and transform into 8 bits integers
    for num in row:
        unpacked = struct.unpack('>I', struct.pack('>f', num))
        unpacked = struct.pack('>I', unpacked[0] << 1)
        uintegers = [np.uint8(c) for c in unpacked[:2]]
        res.extend(uintegers)
    return res
